...
export default function AddBox({ navigation }) {
  ...
  const warehouseOptions = useMemo(
    // Warehouse.name is typed as `unknown` because the Warehouse
    // type is `{id} & Record<string, unknown>`; every doc field
    // is a runtime bag. We know the warehouse has a `name`
    // string (it's a required input field), so cast it.
    () => warehouses.map((wh) => ({ key: wh.id, label: wh.name as string })),
    [warehouses]
  );
  ...
  const handleAddBox = async () => {
    ...
    for (const [k, v] of Object.entries(contents)) {
      ...
      // output, so read it off the raw `v` shape. The form state
